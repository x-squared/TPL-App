...
### 2) Clean DB content but keep SQLite file (optional, quick reset)

```{bash}
cd /Users/stephan/Workspace/TPL-App/backend
source .venv/bin/activate
python scripts/clean_sqlite_keep_file.py
```

### 3) Start backend (includes DB initialization + seed profile handling)
...
### Reset database (delete file, then recreate on next backend start)

```{bash}
cd /Users/stephan/Workspace/TPL-App
rm -f database/tpl_app.db
```
...
cd /Users/stephan/Workspace/TPL-App/backend
source .venv/bin/activate
python scripts/clean_sqlite_keep_file.py --db /Users/stephan/Workspace/TPL-App/database/tpl_app.db
```
