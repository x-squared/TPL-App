...
- `view.donors` allows viewing donor/coordinations data
- `edit.donors` allows changing donor/coordinations data

So a user can be read-only for Donors when they have `view.donors` but not `edit.donors`.
...
8. Login as user with `view.donors` + `edit.donors`:
