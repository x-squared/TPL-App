...
```{bash}
python -m app.db_data --mode <clean|seed|refresh|migrate-procurement-runtime|export-translations-json> --env <DEV|TEST|PROD> [--seed-profile <PROFILE>] [--db-url <URL>]
```
...
```{bash}
python -m app.db_admin --mode <recreate|migrate|refresh|clean|migrate-procurement-runtime> --env <DEV|TEST|PROD> [--seed-profile <PROFILE>] [--db-url <URL>]
```
